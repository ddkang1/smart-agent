...
class TestSmartAgentE2E:
    ...
    async def test_chat_session_with_tools(
        self, mock_get_tool_client, mock_popen, mock_openai, mock_config, mock_process
    ):
        ...
        mock_tool_client.call_function.return_value = {
            "result": "This is a search result for quantum computing."
        }
        ...
        with patch("builtins.input", side_effect=user_inputs):
            with patch("builtins.print") as mock_print:
                ...
                with patch(
                    "asyncio.run",
                    side_effect=lambda coro: asyncio.get_event_loop().run_until_complete(
                        coro
                    ),
                ):
                    # Run chat session
